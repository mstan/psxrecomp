...
void gte_rtps_internal(GTEState* gte, int16_t* V, bool setMac0, uint32_t instr) {
    ...
    if (!s_gte_replay_sandbox) {
         ...
         * garbage shadow back inside the frame. GPU correction still requires
         * same-cell integer agreement; the wider bound exists only for exact
         * NCLIP disagreement telemetry. */
